% This code simulates a multipass cell and aims to duplicate Fig.1 of the
% reference paper:
%
% Kadiwala et al., "Enhanced Nonlinear Spectral Broadening in Multi-Pass Cells Using Molecular Gases," EPJ Web Conf. 267, 02032 (2022)
%
...
r_max = 3e-3; % the maximum radius; half of the spatial window
...
sim.photoionization_model = false;%true;
...
num_roundtrip = 34;
MPC_length = 0.13; % m
focal_R = 0.1; % m; radius of curvature of the mirror
focal_length = focal_R/2; % m
mirror_dispersion = -5.7e-6; % ps^2

plate_z = MPC_length/2 + (1:num_roundtrip*2)*MPC_length;

%% Initial condition
MFD0 = 2*((focal_R^2-(MPC_length-focal_R)^2)*sim.lambda0^2/4/pi^2)^(1/4);
tfwhm = 0.25; % ps
energy = 15e3; % nJ
...
spectrum = abs(fftshift(ifft(prop_output.field(:,:,end)),1)).^2./lambda.^2; % "./lambda" is to make it into the wavelength domain (not with the correct unit but the correct relative strength; we'll normalize it later)
avg_spectrum = trapz(r(r_idx),spectrum(:,r_idx,end).*r(r_idx),2);
avg_spectrum = avg_spectrum/max(avg_spectrum); % normalized
figure;
plot(lambda,avg_spectrum,'Color','b','linewidth',3);
