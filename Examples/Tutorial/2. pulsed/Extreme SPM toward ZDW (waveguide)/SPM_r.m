...
%
% Note that compared to other "SPM(waveguide)" example, there is some
% inconsistency between the 3D-UPPE and the GNLSE results. Possible reason
% might be the effective index of the mode. The GNLSE script computes the
% propagation constant through the Sellmeier's equation of silica, which
% doesn't consider the waveguide effect. On the other hand, 3D-UPPE
% considers the waveguide effect during propagation. As a result, 3D-UPPE
% result should be the more-accurate one.
