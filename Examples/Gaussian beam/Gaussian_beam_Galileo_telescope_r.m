% This code simulates a Gaussian beam passing through a Galileo telescope 
% with a convex and a concave lenses.
...
%Ef_out = add_spherical_lens_phase_r(ifft(prop_output1.field,[],1),prop_output1.r,fftshift(lambda,1)/1e9,convex_radius_of_curvature);
Ef_out = add_thin_lens_phase_r(ifft(prop_output1.field,[],1),prop_output1.r,fftshift(lambda,1)/1e9,convex_focal_length);
...
%Ef_out = add_spherical_lens_phase_r(ifft(prop_output2.field,[],1),prop_output2.r,fftshift(lambda,1)/1e9,concave_radius_of_curvature);
Ef_out = add_thin_lens_phase_r(ifft(prop_output2.field,[],1),prop_output2.r,fftshift(lambda,1)/1e9,concave_focal_length);
