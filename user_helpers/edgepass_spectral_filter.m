...
if ~isstruct(input)
    error('Unlike most auxiliary functions, edgepass_spectral_filter requires that the input be a struct with at least field and dt');
end
...
output = input;
output.field = fft(mult_factor.*ifft(input_field,[],1),[],1);
...
if verbose
    ...
    spectrum = abs(fftshift(ifft(input_field(:,1),[],1),1)).^2*factor_correct_unit.*factor;
